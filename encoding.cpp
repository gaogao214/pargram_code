...
#if __APPLE__
// NSString Cocoa 版本
    unsigned int utf82ucsle(const char* _pUTF8, size_t _nDataLen, char** _ppUnicode )
    {
...
#ifdef _WIN32
// Win32 Api 版本
	GxU32 utf82ucsle(const char* _pUTF8, size_t _nDataLen, char** _ppUnicode )
	{
		int bytes = ::MultiByteToWideChar( CP_UTF8, 0, _pUTF8, (int)_nDataLen, NULL, 0) * 2;
		adjust_conv_buffer(bytes + 2);
		CONV_BUFF.assign(bytes + 2, 0);
		MultiByteToWideChar( CP_UTF8, 0, _pUTF8, (int)_nDataLen , (LPWSTR)CONV_BUFF.data(), bytes );
		*_ppUnicode = (char*)CONV_BUFF.data();
		return bytes+2;
	}

	GxU32 ucsle2utf8( const char * _pUnic, size_t _nDataLen, char ** _ppUTF )
	{
	...
	GxU32 ucsle2gbk( const char * _pUnic, size_t _nDataLen, char ** _ppAscii )
	{
	...
	GxU32 gbk2utf8(const char * pAscii, size_t _nDataLen, char ** _ppUTF)
	{
	...
	uint32_t utf82ucsle(const char* _pUTF8, size_t _nDataLen, char** _ppUnicode )
	{
